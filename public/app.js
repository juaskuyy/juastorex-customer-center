...
checkForm.addEventListener("submit", async (event) => {
  ...
  setMessage(message, "Memeriksa data...");
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(result.error || "Data order tidak ditemukan.");
    }
    ...
    setMessage(message, "Data order ditemukan.", "success");
...
claimPhoto.addEventListener("change", () => {
  ...
  if (!file) {
    photoText.textContent = "Pilih foto atau screenshot";
    return;
  }
...
claimForm.addEventListener("submit", async (event) => {
  ...
  if (!currentOrder) {
    setMessage(
      claimMessage,
      "Silakan cek order terlebih dahulu.",
      "error"
    );
    return;
  }
  ...
  const photo = claimPhoto.files?.[0];

  if (!reason || !photo) {
    setMessage(
      claimMessage,
      "Alasan dan foto wajib diisi.",
      "error"
    );
    return;
  }
  ...
  claimButton.textContent = "Mengirim...";

  setMessage(
    claimMessage,
    "Mohon tunggu, klaim sedang dikirim."
  );
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        result.error || "Klaim gagal dikirim."
      );
    }
    ...
    claimButton.textContent = "Kirim klaim";
  }
});

window.addEventListener("load", async () => {
  if (!autoCode || !autoKey) return;

  try {
    setMessage(message, "Memuat data order...");

    const response = await fetch(
      `${API}/api/order?code=${encodeURIComponent(autoCode)}&key=${encodeURIComponent(autoKey)}`
    );
    ...
    if (!response.ok) {
      throw new Error(
        result.error || "Order tidak ditemukan."
      );
    }
